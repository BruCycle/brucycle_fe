...
#   so that I can login to the application using my Strava account
  ...
  describe 'it has attributes' do 
    it 'has app name and link to OAuth' do 
      ...
      expect(page).to have_link('Login with Strava', href: "http://www.strava.com/oauth/authorize?client_id=#{ENV["strava_client_id"]}&response_type=code&redirect_uri=http://www.example.com/exchange_token&approval_prompt=force&scope=read,activity:read_all")
    end
  end
